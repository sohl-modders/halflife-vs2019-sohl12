...
void CDrillSergeant::Spawn()
{
	...
	MonsterInit();
	SetUse(&CDrillSergeant::FollowerUse);
}
...
void CDrillSergeant::TalkInit()
{
	...
	if (!m_iszSpeakAs)
	{
		...
		if (pev->spawnflags & SF_MONSTER_PREDISASTER) //LRC
			m_szGrp[TLK_USE] = "DR_PFOLLOW";
		else
			m_szGrp[TLK_USE] = "DR_OK";

		if (pev->spawnflags & SF_MONSTER_PREDISASTER)
			m_szGrp[TLK_UNUSE] = "DR_PWAIT";
		else
		...
		m_szGrp[TLK_PHELLO] = nullptr; //"DR_PHELLO";		// UNDONE
		m_szGrp[TLK_PIDLE] = nullptr; //"DR_PIDLE";			// UNDONE
